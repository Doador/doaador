...
if "Ops" not in st.session_state:
    st.session_state.ops = [
        "Doações", "Importância", "Requisitos para Doação",
        "Etapas para Doação", "Elegibilidade de Doação", 
        'Agendamento', "Cadastro para Voluntários", 'Vídeo Informativo', 'Agradecimentos'
    ]
...
def MudarH():
    st.session_state.Nav = 'Vídeo Informativo'
def MudarI():
    st.session_state.Nav = 'Agradecimentos'
...
if pagina == "Elegibilidade de Doação":
    ...
    with st.form("form_agendamento"):
        ...
        if submit_agendamento:
            if not (nome):
                ...
                salvar_agendamento(nome, idade, sexo, tipo_doacao, tipo_sanguineo, data_hora, hora, notas)
    ...
    st.button('Ir para Vídeo Informativo', type="primary", on_click=MudarH)



elif pagina == "Vídeo Informativo":
    st.title("Vídeo Explicativo sobre a Doação de Sangue, Medula Óssea e Órgãos.")
    
    st.video("Vídeo.mp4")
    st.button('Ir para Agradecimentos', type="primary", on_click=MudarI)

elif pagina == "Agradecimentos":
    add_bg_image("imagemfim.png")
    st.button('Voltar para Doações', type="primary", on_click=MudarA)
